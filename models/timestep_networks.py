...
class AttentionPool1d(nn.Module):
    def __init__(self, temporal_dim: int, embed_dim: int, num_heads: int, output_dim: int = None):
        super().__init__()
        self.positional_embedding = nn.Parameter(th.randn(temporal_dim + 1, embed_dim) / embed_dim ** 0.5)
    ...
    def forward(self, x):
        ...
        x = th.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (L+1)NC
        ...
        return x.permute(1, 0, 2)  # [batch_size, 1, short]

class AttentionPooledUNet(nn.Module):
    def __init__(
        self,
        image_size,
        concat_size,
        ...
        self.noise_size = image_size + concat_size
        ...
        self.out = nn.Sequential(
            normalization(ch),
            nn.SiLU(),
            AttentionPool1d(self.noise_size, model_channels, 4, self.image_size),
        )
