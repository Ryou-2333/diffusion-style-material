...
def combine_to_gif(dir, formatter):
    print(f"Generating gif of {dir}")
    images = []
    file_list = glob.glob(dir + '/' + formatter)
    file_list.sort(key=natural_keys)
    for file in file_list:
        images.append(imageio.imread(file))

    imageio.mimsave(dir + '/output.gif', images, duration=50)

def combine_to_mp4(dir, formatter):
    print(f"Generating video of {dir}")
    writer = imageio.get_writer(dir + '/output.mp4', fps=30)
    file_list = glob.glob(dir + '/' + formatter)
    file_list.sort(key=natural_keys)
    for file in file_list:
